...
def get_Gatepass_customer_code_summary(date1_from, date1_to, cnge_code_filter=None):
    if cnge_code_filter:
    ...
    try:
        ...
        SELECT V.VNO, V.VDATE, CNGe.CODE, CNGe.NAME,
               V.REFNO, 'BILL' AS DOCUMENTTYPE, BH.BILLNO AS DOCUMENTNO,
               V.DRAMOUNT, 0 AS CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(BH.BILLAMOUNT AS DECIMAL(18,2)) AS BILLAMOUNT
        FROM #VOUCHER V
        ...
        INSERT INTO #VoucherDetails
        SELECT V.VNO, V.VDATE, CNGe.CODE, CNGe.NAME,
               V.REFNO, 'BILL CANCELLED' AS DOCUMENTTYPE, BH.BILLNO AS DOCUMENTNO,
               0 AS DRAMOUNT, V.CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(-1 * BH.BILLAMOUNT AS DECIMAL(18,2)) AS BILLAMOUNT
        FROM #VOUCHER V
        ...
        INSERT INTO #VoucherDetails
        SELECT V.VNO, V.VDATE, CNGe.CODE, CNGe.NAME,
               V.REFNO, 'MR' AS DOCUMENTTYPE, CAST(MD.MRNO AS VARCHAR) AS DOCUMENTNO,
               0 AS DRAMOUNT, V.CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(
                   -1 * (
                       ISNULL(MD.RECDAMOUNT, 0) +
                       ISNULL(MD.TDSAMOUNT, 0) +
                       ISNULL(MD.CLAIM, 0) +
                       ISNULL(MD.BANKCHARGES, 0)
                   ) AS DECIMAL(18,2)
               ) AS BILLAMOUNT
        FROM #VOUCHER V
        ...
        INSERT INTO #VoucherDetails
        SELECT V.VNO, V.VDATE, CNGe.CODE, CNGe.NAME,
               V.REFNO, 'REVERSE MR' AS DOCUMENTTYPE, CAST(BH.MRNO AS VARCHAR) AS DOCUMENTNO,
               V.DRAMOUNT, 0 AS CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(
                   ISNULL(MH.RECDAMOUNT, 0) +
                   ISNULL(MH.TDSAMOUNT, 0) +
                   ISNULL(MH.CLAIM, 0) +
                   ISNULL(MH.BANKCHARGES, 0)
                   AS DECIMAL(18,2)
               ) AS BILLAMOUNT
        FROM #VOUCHER V
        ...
        INSERT INTO #VoucherDetails
        SELECT V.VNO, V.VDATE, CNGe.CODE, CNGe.NAME,
               CAST(V.REFNO AS VARCHAR) AS REFNO, 'CUSTOMER REFUND' AS DOCUMENTTYPE,
               CAST(CR.MRNO AS VARCHAR) AS DOCUMENTNO,
               V.DRAMOUNT, 0 AS CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(V.DRAMOUNT AS DECIMAL(18,2)) AS BILLAMOUNT
        FROM #VOUCHER V
        INNER JOIN CUSTOMERREFUND CR WITH (NOLOCK) ON V.VNO = CR.VNO
        LEFT JOIN MRHEAD MH WITH (NOLOCK) ON MH.MRNO = CR.MRNO
        OUTER APPLY (
            SELECT TOP 1 MD.CNGECODE CODE, CNGE.NAME
            FROM MRDETAIL MD WITH (NOLOCK)
            LEFT JOIN CNGRCNGEMAST CNGE ON CNGE.CODE = MD.CNGECODE
            WHERE MD.MRNO = MH.MRNO
              AND MD.CNGECODE IS NOT NULL
        ) CNGe
        WHERE V.LEDCODE = @LedgerCode
          AND CR.CANCEL <> 'Y'
          AND V.DRAMOUNT > 0
        ...
        SELECT V.VNO, V.VDATE, CBH.CNGECODE AS CODE, BCNGE.NAME AS NAME,
               REPLACE(V.REFNO, 'CREDIT NOTE # ', '') AS REFNO,
               'CREDIT NOTE MR1' AS DOCUMENTTYPE,
               CAST(CRNO.MRNO AS VARCHAR) AS DOCUMENTNO,
               0 AS DRAMOUNT, V.CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(-1 * CMD.REBATE AS DECIMAL(18,2)) AS BILLAMOUNT
        FROM #VOUCHER V
        INNER JOIN MRHEAD CRNO WITH(NOLOCK)
            ON CRNO.CREDITNOTENO = REPLACE(V.REFNO, 'CREDIT NOTE # ', '')
           AND CRNO.CANCEL <> 'Y'
        ...
        SELECT V.VNO, V.VDATE, CNGe.CODE AS CODE, CNGe.NAME AS NAME,
               REPLACE(V.REFNO, 'CREDIT NOTE # ', '') AS REFNO,
               'CREDIT NOTE MR' AS DOCUMENTTYPE,
               CAST(MD.MRNO AS VARCHAR) AS DOCUMENTNO,
               0 AS DRAMOUNT, V.CRAMOUNT,
               V.BRANCHCODE, V.BRANCHNAME,
               CAST(-1 * MD.REBATE AS DECIMAL(18,2)) AS BILLAMOUNT
        FROM #VOUCHER V
        INNER JOIN MRHEAD MH WITH (NOLOCK) ON MH.VNO = V.VNO
        INNER JOIN MRDETAIL MD WITH (NOLOCK) ON MD.MRNO = MH.MRNO AND MH.CANCEL <> 'y'
        INNER JOIN BILLHEAD BH WITH (NOLOCK) ON BH.BILLNO = MD.BILLNO AND BH.CANCEL <> 'Y'
        ...
        -- BL DEBIT CREDIT NOTE
        INSERT INTO #VoucherDetails
        SELECT v.vno, v.vdate, CNGe.CODE, CNGe.NAME, v.REFNO,
               'BL-DRCRNOTE' AS DOCUMENTTYPE, 
               SUBSTRING(v.REFNO,
                         CHARINDEX('INVOICE # ', v.REFNO) + 10,
                         CHARINDEX(' - ', v.REFNO) - (CHARINDEX('INVOICE # ', v.REFNO) + 10)
               ) AS DOCUMENTNO, 
               v.DRAMOUNT, v.CRAMOUNT,
               v.BRANCHCODE, v.BRANCHNAME,
               CASE 
                   WHEN v.DRAMOUNT > 0 THEN CAST(v.DRAMOUNT AS DECIMAL(18,2))
                   WHEN v.CRAMOUNT > 0 THEN CAST(-1 * v.CRAMOUNT AS DECIMAL(18,2))
                   ELSE 0
               END AS BILLAMOUNT
        FROM #VOUCHER v
        INNER JOIN BILLHEAD BH WITH (NOLOCK)
            ON BH.billno = SUBSTRING(
                v.REFNO,
                CHARINDEX('INVOICE # ', v.REFNO) + 10,
                CHARINDEX(' - ', v.REFNO) - (CHARINDEX('INVOICE # ', v.REFNO) + 10)
            )
        ...
        SELECT v.vno, v.vdate, NULL AS code, NULL AS name,
               V.REFNO, 'DIRECT' AS DOCUMENTTYPE, NULL AS DOCUMENTNO,
               v.DRAMOUNT, v.CRAMOUNT,
               v.BRANCHCODE, v.BRANCHNAME,
               CASE 
                   WHEN v.DRAMOUNT > 0 THEN CAST(v.DRAMOUNT AS DECIMAL(18,2))
        ...
        CREATE TABLE #VoucherSummary (
            CODE VARCHAR(50),
            NAME VARCHAR(255),
            TOTAL_BILLAMOUNT DECIMAL(18, 2)
        );

        -- Insert aggregated data into #VoucherSummary
        INSERT INTO #VoucherSummary
        SELECT CODE, NAME, SUM(BILLAMOUNT) AS TOTAL_BILLAMOUNT
        FROM #VoucherDetails
        GROUP BY CODE, NAME
        HAVING SUM(BILLAMOUNT) <> 0;

        -- Select from the summary table
        SELECT * FROM #VoucherSummary;

        -- Drop the temporary tables
        DROP TABLE #VoucherDetails;
        DROP TABLE #VoucherSummary;
        """
