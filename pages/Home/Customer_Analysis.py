...
# NBD Customer MIS
# =====================================================
def _nbd_first_existing_column(df: pd.DataFrame, candidates) -> str | None:
    """Return the first available source column using case/space-insensitive matching."""
    ...
    for candidate in candidates:
        key = "".join(ch for ch in str(candidate).casefold() if ch.isalnum())
        if key in lookup:
            return lookup[key]
    return None


def _nbd_first_non_blank(series: pd.Series):
    if series is None:
        return ""
...
def _nbd_period_bounds(fin_year: str, selected_quarters, selected_months):
    """Resolve the current report period from the dashboard Quarter/Month filters."""
    from calendar import monthrange
    from datetime import date

    fy_start, fy_end = [int(x) for x in fin_year.split("-")]
    month_to_fin = {name: idx + 1 for idx, name in enumerate(MONTH_ORDER)}
    quarter_to_fin = {
        "Q1": [1, 2, 3],
        "Q2": [4, 5, 6],
        "Q3": [7, 8, 9],
    ...
    if selected_months:
        fiscal_months = sorted({month_to_fin[m] for m in selected_months if m in month_to_fin})
    elif selected_quarters:
        fiscal_months = sorted({m for q in selected_quarters for m in quarter_to_fin.get(q, [])})
    else:
        fiscal_months = list(range(1, 13))

    if not fiscal_months:
        fiscal_months = list(range(1, 13))

    def to_calendar(fin_month: int):
        if 1 <= fin_month <= 9:  # Apr-Dec
            return fy_start, fin_month + 3
        return fy_end, fin_month - 9  # Jan-Mar

    sy, sm = to_calendar(min(fiscal_months))
    ey, em = to_calendar(max(fiscal_months))
    return date(sy, sm, 1), date(ey, em, monthrange(ey, em)[1])


def build_nbd_customer_mis(
    current_df: pd.DataFrame,
    compare_df: pd.DataFrame,
    code_col: str,
    name_col: str,
) -> pd.DataFrame:
    """Build the NBD MIS union of current-period and comparison-period customers.

    Customer Type logic mirrors the shared MIS sample:
      NEW     -> current business > 0 and comparison business = 0
      LOST    -> comparison business > 0 and current business = 0
      REGULAR -> business exists in both periods
    """
    current_df = current_df.copy()
    compare_df = compare_df.copy()

    # Customer master fields are optional because some versions of the stored
    # procedure do not return mobile/GST columns. The report still renders and
    # leaves those fields blank when they are unavailable.
    mobile_candidates = [
        "CustomerMobileNo", "Customer Mobile NO", "CustomerMobile", "MobileNo",
        "Mobile", "MobileNumber", "ContactNo", "ContactNumber", "PhoneNo", "Phone",
        "cngrmobile", "cngrmobileno", "cngemobile", "cngemobileno",
    ]
    gst_candidates = [
        "GSTNo", "GST No", "GSTIN", "GSTNumber", "GST No.", "gst_no", "gstin_no",
        "cngrgst", "cngrgstno", "cngegst", "cngegstno",
    ]

    mobile_col = _nbd_first_existing_column(current_df, mobile_candidates) or _nbd_first_existing_column(compare_df, mobile_candidates)
    gst_col = _nbd_first_existing_column(current_df, gst_candidates) or _nbd_first_existing_column(compare_df, gst_candidates)

    def summarize(frame: pd.DataFrame, prefix: str) -> pd.DataFrame:
        if frame is None or frame.empty:
            return pd.DataFrame(columns=[
                code_col, name_col, "Zone", "Circle", "Branch", "Customer Mobile NO", "GST No",
                f"{prefix} Sale", f"{prefix} LTL Sale", f"{prefix} FTL Sale",
            ])

        work = frame.copy()
        work["Revenue"] = pd.to_numeric(work.get("Revenue", 0), errors="coerce").fillna(0.0)
        load = work.get("LoadType", pd.Series("", index=work.index)).fillna("").astype(str).str.upper().str.strip()
        work["__ltl_sale"] = work["Revenue"].where(load.str.contains("LTL", na=False), 0.0)
        work["__ftl_sale"] = work["Revenue"].where(load.str.contains("FTL", na=False), 0.0)

        group_cols = [code_col]
        grouped_sales = work.groupby(group_cols, dropna=False, as_index=False).agg(
            **{
                f"{prefix} Sale": ("Revenue", "sum"),
                f"{prefix} LTL Sale": ("__ltl_sale", "sum"),
                f"{prefix} FTL Sale": ("__ftl_sale", "sum"),
            }
        )

        detail_cols = [code_col]
        for col in [name_col, "Zone", "Circle", "Branch"]:
            if col in work.columns and col not in detail_cols:
                detail_cols.append(col)
        if mobile_col and mobile_col in work.columns and mobile_col not in detail_cols:
            detail_cols.append(mobile_col)
        if gst_col and gst_col in work.columns and gst_col not in detail_cols:
            detail_cols.append(gst_col)

        details = work[detail_cols].groupby(code_col, dropna=False, as_index=False).agg(_nbd_first_non_blank)
        details = details.rename(columns={
            mobile_col: "Customer Mobile NO" if mobile_col else mobile_col,
            gst_col: "GST No" if gst_col else gst_col,
        })
        if "Customer Mobile NO" not in details.columns:
            details["Customer Mobile NO"] = ""
        if "GST No" not in details.columns:
            details["GST No"] = ""
        for col in [name_col, "Zone", "Circle", "Branch"]:
            if col not in details.columns:
                details[col] = ""

        return details.merge(grouped_sales, on=code_col, how="outer")

    cur = summarize(current_df, "Current")
    cmp = summarize(compare_df, "Compare")

    detail_fields = [name_col, "Zone", "Circle", "Branch", "Customer Mobile NO", "GST No"]
    merged = cmp.merge(cur, on=code_col, how="outer", suffixes=("__cmp", "__cur"))

    for field in detail_fields:
        cmp_field = f"{field}__cmp"
        cur_field = f"{field}__cur"
        cur_values = merged[cur_field] if cur_field in merged.columns else pd.Series("", index=merged.index)
        cmp_values = merged[cmp_field] if cmp_field in merged.columns else pd.Series("", index=merged.index)
        cur_values = cur_values.fillna("").astype(str).str.strip()
        cmp_values = cmp_values.fillna("").astype(str).str.strip()
        merged[field] = cur_values.where(cur_values.ne(""), cmp_values)

    sale_cols = [
        "Compare Sale", "Compare LTL Sale", "Compare FTL Sale",
        "Current Sale", "Current LTL Sale", "Current FTL Sale",
    ]
    for col in sale_cols:
        merged[col] = pd.to_numeric(merged.get(col, 0), errors="coerce").fillna(0.0)

    merged = merged[(merged["Compare Sale"] != 0) | (merged["Current Sale"] != 0)].copy()
    merged["Customer Type"] = "REGULAR"
    merged.loc[(merged["Compare Sale"] == 0) & (merged["Current Sale"] != 0), "Customer Type"] = "NEW"
    merged.loc[(merged["Compare Sale"] != 0) & (merged["Current Sale"] == 0), "Customer Type"] = "LOST"

    output = merged[[
        "Zone", "Circle", "Branch", name_col, "Customer Mobile NO", "GST No",
        "Compare Sale", "Compare LTL Sale", "Compare FTL Sale",
        "Current Sale", "Current LTL Sale", "Current FTL Sale", "Customer Type",
    ]].copy()
    output = output.rename(columns={name_col: "Customer"})
    output["Customer"] = output["Customer"].fillna("").astype(str).str.strip()
    output = output.sort_values(["Zone", "Circle", "Branch", "Customer"], kind="stable").reset_index(drop=True)
    output.insert(0, "S#", range(1, len(output) + 1))
    return output
...
def render_nbd_customer_tab(
    ...
    new_count = int((nbd_df["Customer Type"] == "NEW").sum())
    lost_count = int((nbd_df["Customer Type"] == "LOST").sum())
    regular_count = int((nbd_df["Customer Type"] == "REGULAR").sum())
    total_count = len(nbd_df)
    ...
    with m4:
        kpi_card("Regular", f"{regular_count:,}", "Business in both periods", "🔁", "#7c3aed", None)

    dashboard_spacer(8)

    # Add a total line like the reference MIS report.
    sale_cols = [
        "Compare Sale", "Compare LTL Sale", "Compare FTL Sale",
        "Current Sale", "Current LTL Sale", "Current FTL Sale",
    ]
    total_row = {col: "" for col in nbd_df.columns}
    total_row["Customer"] = "TOTAL"
    for col in sale_cols:
        total_row[col] = float(nbd_df[col].sum())
    display_df = pd.concat([nbd_df, pd.DataFrame([total_row])], ignore_index=True)
    ...
    # NBD-specific export mirrors the shared Excel column layout and title rows.
    output = BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        start_row = 4
        display_df.to_excel(writer, sheet_name="NBD MIS", index=False, startrow=start_row)
        workbook = writer.book
        ...
        worksheet.write(0, 0, "SUGAM PARIVAHAN PVT. LTD.", title_fmt)
        ...
        for idx, col_name in enumerate(display_df.columns):
            ...
            if col_name == "Customer":
                width = 34
            elif col_name in {"Zone", "Circle", "Branch"}:
                width = 20
        ...
        total_excel_row = start_row + len(display_df)
        for col_idx in range(len(display_df.columns)):
            if display_df.columns[col_idx] in sale_cols:
                worksheet.write_number(total_excel_row, col_idx, float(display_df.iloc[-1, col_idx] or 0), total_fmt)
            else:
                worksheet.write(total_excel_row, col_idx, display_df.iloc[-1, col_idx], total_fmt)
