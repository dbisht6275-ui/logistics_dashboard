...
# =========================================================
# THREE-PERIOD DELIVERY SUMMARY
# ORIGINAL SQL QUERY RETAINED
# =========================================================
def get_Delivery_summary_trunover(
    date1_from,
    date1_to,
    date2_from,
    date2_to,
    ...
    try:
        ...
        st.error(
            f"Delivery Summary Turnover report error: {error}"
        )
        return pd.DataFrame()


# =========================================================
# ONE-YEAR DELIVERY SUMMARY
# =========================================================
def get_delivery_summary_one_year_turnover(
    year_from,
    year_to,
):
    try:
        ...
        st.error(
            f"Delivery Summary one-year report error: {error}"
        )
        return pd.DataFrame()
...
# REPORT DISPLAY
# =========================================================
def display_delivery_summary_report(
    df,
    report_type,
):
    if df is None or df.empty:
    ...
    try:
        ...
        AgGrid(
            ...
            key=f"delivery_summary_grid_{report_type}",
        )
    ...
    if report_type == "One Year Average":
        file_name = "DeliverySummaryOneYearTurnover.xlsx"
        export_key = "delivery_summary_one_year_export"
    else:
        file_name = "DeliverySummaryTurnover.xlsx"
        export_key = "delivery_summary_comparison_export"
...
# MAIN UI
# =========================================================
def show_delivery_summary_turnover():
    apply_report_css()
    ...
    st.markdown(
        ...
        'Delivery Summary Turnover Report'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        'Destination-zone-wise LTL, FTL and total '
        'delivery turnover analysis'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    report_type = st.radio(
        ...
        horizontal=True,
        key="delivery_summary_report_type",
    )
    ...
    if report_type == "Period Comparison":
        ...
        with col3:
            ...
            d3_to = st.date_input(
                ...
                format="DD-MM-YYYY",
                key="delivery_summary_p3_to",
            )

        if st.button(
            "Generate Report",
            use_container_width=True,
            key="generate_delivery_summary_comparison",
        ):
            with st.spinner(
                "Generating Delivery Summary report..."
            ):
                df = get_Delivery_summary_trunover(
                    d1_from,
                    d1_to,
                    d2_from,
                    d2_to,
            ...
            display_delivery_summary_report(
                df,
                report_type,
            )
        ...
        with year_col1:
            year_from = st.date_input(
                ...
                format="DD-MM-YYYY",
                key="delivery_summary_year_from",
            )
        ...
        with year_col2:
            year_to = st.date_input(
                ...
                value=defaults["today"],
                format="DD-MM-YYYY",
                key="delivery_summary_year_to",
            )

        if st.button(
            "Generate One Year Report",
            use_container_width=True,
            key="generate_delivery_summary_one_year",
        ):
            with st.spinner(
                "Generating one-year Delivery Summary report..."
            ):
                df = (
                    get_delivery_summary_one_year_turnover(
                        year_from,
                        year_to,
                    )
                )
...
# FUNCTION ALIASES
# =========================================================
def show_DeliverySummaryTurnover():
    show_delivery_summary_turnover()


def show_Delivery_summary_trunover():
    show_delivery_summary_turnover()