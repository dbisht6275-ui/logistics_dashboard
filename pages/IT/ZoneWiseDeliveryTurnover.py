...
def get_zone_wise_delivery_turnover(
    ...
    try:
        ...
        st.error(
            f"Zone Wise Delivery Turnover report error: {error}"
        )
        return pd.DataFrame()
...
# ONE-YEAR REPORT
# =========================================================
def get_zone_wise_delivery_one_year_turnover(
    year_from,
    year_to,
):
    try:
        ...
        st.error(
            f"Zone Wise Delivery one-year report error: {error}"
        )
        return pd.DataFrame()
...
def display_delivery_report(
    ...
    try:
        ...
        AgGrid(
            ...
            key=f"zone_delivery_grid_{report_type}",
        )
    ...
    if report_type == "One Year Average":
        file_name = "ZoneWiseDeliveryOneYearTurnover.xlsx"
        export_key = "zone_delivery_one_year_export"
    else:
        file_name = "ZoneWiseDeliveryTurnover.xlsx"
        export_key = "zone_delivery_comparison_export"
...
# MAIN UI FUNCTION
# =========================================================
def show_zone_wise_delivery_turnover():
    apply_report_css()
    ...
    st.markdown(
        ...
        'Zone Wise Delivery Turnover Report'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        'Destination-zone-wise LTL, FTL, total and Nepal '
        'delivery analysis'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    if report_type == "Period Comparison":
        ...
        with col3:
            ...
            d3_to = st.date_input(
                ...
                format="DD-MM-YYYY",
                key="zone_delivery_p3_to",
            )

        if st.button(
            "Generate Report",
            use_container_width=True,
            key="generate_zone_delivery_comparison",
        ):
            with st.spinner(
                "Generating Zone Wise Delivery report..."
            ):
                df = get_zone_wise_delivery_turnover(
                    d1_from,
                    d1_to,
                    d2_from,
                    d2_to,
        ...
        with year_col2:
            year_to = st.date_input(
                ...
                value=defaults["today"],
                format="DD-MM-YYYY",
                key="zone_delivery_year_to",
            )

        if st.button(
            "Generate One Year Report",
            use_container_width=True,
            key="generate_zone_delivery_one_year",
        ):
            with st.spinner(
                "Generating one-year Delivery report..."
            ):
                df = (
                    get_zone_wise_delivery_one_year_turnover(
                        year_from,
                        year_to,
                    )
                )
...
# FUNCTION ALIASES
# =========================================================
def show_ZoneWiseDeliveryTurnover():
    show_zone_wise_delivery_turnover()


def show_ZoneDeliveryTurnover():
    show_zone_wise_delivery_turnover()