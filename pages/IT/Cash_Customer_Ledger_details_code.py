...
from reports.CashCustomerVoucherDetailscode import get_Cash_Customer_Ledger_details_code
...
def load_report(content):
    ...
    ctk.CTkLabel(
        frame, 
        text="📊 Cash Customer Code Ledger Details", 
    ...
    ctk.CTkLabel(row_frame, text="CNGR Code (Optional)", font=ctk.CTkFont(size=14)).grid(row=1, column=0, padx=20, pady=10, sticky="w")
    ...
    def generate():
        try:
            ...
            columns, rows = get_Cash_Customer_Ledger_details_code(from_date, to_date, cnge_code)

            # Show report in Treeview
            show_treeview(content, columns, rows, "Cash Customer Code Ledger")
