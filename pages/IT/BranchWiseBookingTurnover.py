...
# =========================================================
# THREE-PERIOD BRANCH BOOKING REPORT
# ORIGINAL SQL QUERY RETAINED
# =========================================================
def get_Branch_wise_booking_turnover(
    date1_from,
    date1_to,
    date2_from,
    date2_to,
    ...
    try:
        ...
        st.error(
            f"Branch Wise Booking Turnover report error: "
            f"{error}"
        )
        return pd.DataFrame()
...
# =========================================================
# ONE-YEAR BRANCH BOOKING REPORT
# =========================================================
def get_branch_wise_booking_one_year_turnover(
    year_from,
    year_to,
):
    try:
        ...
        st.error(
            f"Branch Wise Booking one-year report error: "
            f"{error}"
        )
        return pd.DataFrame()
...
# REPORT DISPLAY
# =========================================================
def display_branch_booking_report(
    df,
    report_type,
):
    if df is None or df.empty:
    ...
    try:
        ...
        AgGrid(
            ...
            key=f"branch_booking_grid_{report_type}",
        )
    ...
    if report_type == "One Year Average":
        file_name = (
            "BranchWiseBookingOneYearTurnover.xlsx"
        )
        export_key = (
            "branch_booking_one_year_export"
        )
    else:
        file_name = "BranchWiseBookingTurnover.xlsx"
        export_key = (
            "branch_booking_comparison_export"
        )
...
# MAIN UI
# =========================================================
def show_branch_wise_booking_turnover():
    apply_report_css()
    ...
    st.markdown(
        ...
        'Branch Wise Booking Turnover Report'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        'Origin branch-wise Non-FTL and FTL '
        'booking turnover analysis'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    report_type = st.radio(
        ...
        horizontal=True,
        key="branch_booking_report_type",
    )
    ...
    if report_type == "Period Comparison":
        ...
        with col3:
            ...
            d3_to = st.date_input(
                ...
                format="DD-MM-YYYY",
                key="branch_booking_p3_to",
            )

        if st.button(
            "Generate Report",
            use_container_width=True,
            key="generate_branch_booking_comparison",
        ):
            with st.spinner(
                "Generating Branch Wise Booking report..."
            ):
                df = get_Branch_wise_booking_turnover(
                    d1_from,
                    d1_to,
                    d2_from,
                    d2_to,
        ...
        with year_col2:
            year_to = st.date_input(
                ...
                value=defaults["today"],
                format="DD-MM-YYYY",
                key="branch_booking_year_to",
            )

        if st.button(
            "Generate One Year Report",
            use_container_width=True,
            key="generate_branch_booking_one_year",
        ):
            with st.spinner(
                "Generating one-year Branch Booking report..."
            ):
                df = (
                    get_branch_wise_booking_one_year_turnover(
                        year_from,
                        year_to,
                    )
                )
...
# FUNCTION ALIASES
# =========================================================
def show_BranchWiseBookingTurnover():
    show_branch_wise_booking_turnover()
